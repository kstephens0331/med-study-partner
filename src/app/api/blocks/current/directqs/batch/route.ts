...
import { DIRECT_BATCH_PROMPT, parseItems } from "@/lib/ai/material";
...
export async function POST(req:NextRequest){
  ...
  const out = await chat(DIRECT_BATCH_PROMPT(ctx));
  const items = parseItems(out);
  const merged = (lp?.directqs||[]).concat(items);
  await sb.from("lecture_packs").update({ directqs: merged }).eq("lecture_id", lectureId);
