...
-- SAFE VERSION: All statements use IF NOT EXISTS or conditional checks
...
begin
  ...
  -- Study sessions
  if not exists (select 1 from pg_policies where tablename = 'study_sessions' and policyname = 'Users can view own study sessions') then
    create policy "Users can view own study sessions" on study_sessions for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'study_sessions' and policyname = 'Users can insert own study sessions') then
    create policy "Users can insert own study sessions" on study_sessions for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'study_sessions' and policyname = 'Users can update own study sessions') then
